...
export class CalculatorComponent {
  ...
  formclcValue: string = '';
  formclcValueExpression: boolean = false;
  clVal: string = 'AC'
  completePattern = new RegExp("[0-9-+*/.()]");
  numbersPattern = new RegExp('^[0-9]+$');
  formclcBg: string = '#424242';
  ...
  clcBtnPress(inputValue: string) {
      if (this.formclcValue === 'Error' || this.formclcValue === 'NaN') this.formclcValue = '';
      if (this.validateInput(inputValue)) {
          if('/*-+'.indexOf(inputValue) === -1 && !this.formclcValueExpression) {
              this.formclcValue = inputValue;
  ...
  handleKeyboardEvents(keyPress: KeyboardEvent) {
      ...
      switch (key) {
          case '0': this.clcBtnPress(key); break;
          case '1': this.clcBtnPress(key); break;
          case '2': this.clcBtnPress(key); break;
          case '3': this.clcBtnPress(key); break;
          case '4': this.clcBtnPress(key); break;
          case '5': this.clcBtnPress(key); break;
          case '6': this.clcBtnPress(key); break;
          case '7': this.clcBtnPress(key); break;
          case '8': this.clcBtnPress(key); break;
          case '9': this.clcBtnPress(key); break;
          case '+': this.clcBtnPress(key); break;
          case '-': this.clcBtnPress(key); break;
          case '*': this.clcBtnPress(key); break;
          case '/': this.clcBtnPress(key); break;
          case '(': this.clcBtnPress(key); break;
          ...
          case '.': this.clcBtnPress(key); break;
          case '=': this.clcBtnPress(key); break;
          case 'Enter': this.clcBtnPress('='); break;
      }
  }

  // Return true if the next input matches something that makes sense
  validateInput(input) {
      let lastValue = this.formclcValue.substr(this.formclcValue.length - 1, 1);
  ...
  flashLCD() {
      let currentformclcValue = this.formclcValue;
      this.formclcValue = 'Input Error';
      this.formclcBg = '#a02626';
      setTimeout(() => {
          this.formclcBg = '#424242';
          this.formclcValue = currentformclcValue;
      }, 500);
  }
  ...
  clcBtnClear() {
      if (this.formclcValueExpression) {
          let valueToRemove = this.formclcValue.substring(this.formclcValue.length, this.formclcValue.length - 1);
          if ('()'.indexOf(valueToRemove) > -1) {
              this.editParenthesisFlag(valueToRemove, false);
          }
          this.formclcValue = this.formclcValue.slice(0, -1);
          if (!this.formclcValue) this.flipACButton(false);
  ...
  flipACButton(input: boolean) {
      this.formclcValueExpression = input;
      this.clVal = (this.formclcValueExpression) ? 'CE' : 'AC';
  }
  ...
  clcBtnCalculate() {
      let valueToEvaluate: string = this.formclcValue;
      ...
      if (this.completePattern.test(valueToEvaluate) && !this.parenthesisFlag) {
          ...
          this.expressionResult = this.formclcValue + ' =';
          this.formclcValue = this.roundValue(solution);
      } else {
          this.expressionResult = 'Cannot resolve: ' + this.formclcValue;
          this.formclcValue = "Error";
          this.parenthesisFlag = 0;
      }
      this.flipACButton(false);
  }
